...
from Shaders.shaders import sky_shader, water_shader, underwater_shader  # Assuming this is defined elsewhere
from ecology_simulation import EcoSim

class Oasis:
    def __init__(self, ts=10, world_size=2000, height_scale=100, game_start_time=42000):
        # Predefine variables
        self.terrain_subdivisions = ts
        ...
        self.player.position = Vec3(0, 30, 0)
    ...
    def get_terrain_height(self, x, z):
        idx = int(x + self.world_size // 2)
        idz = int(z + self.world_size // 2)
        if 0 <= idx < self.world_size and 0 <= idz < self.world_size:
